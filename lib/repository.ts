...
const request = async (path: string, init?: RequestInit) => { const response = await fetch(path, { ...init, headers: { "Content-Type": "application/json", ...init?.headers } }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "요청을 처리하지 못했습니다."); return data; };
...
export const repository = {
  async listPlans() {
    ...
    const tokens: string[] = JSON.parse(localStorage.getItem(TOKENS_KEY) || "[]");
    const results = await Promise.all(tokens.map((value) => this.getBundle(value).catch(() => null)));
    return results.filter((value): value is PlanBundle => value !== null);
  },
